...
# Description: Automated script for install-flutter-clean.ps1
...
if (Test-Path $DOWNLOAD_PATH) {
    Write-ColorOutput "WARNING: Flutter SDK already downloaded at: $DOWNLOAD_PATH" "Yellow"
...
if (-not (Test-Path $DOWNLOAD_PATH)) {
    ...
    Write-ColorOutput "SUCCESS: Using existing download" "Green"
}
...
try {
    ...
    Write-ColorOutput "`nSUCCESS: Flutter installation verified" "Green"
} catch {
...
Write-ColorOutput "`nIMPORTANT NEXT STEPS:" "Yellow"
...
Write-ColorOutput "`nHappy Flutter Development!" "Green"
